...
- [x] **Fase 4** — Endpoint de presença enviada (`src/app/api/whatsapp/presence/route.ts`)
...
## Riscos técnicos — status após implementação
- **`presence.update`** (webhook): implementado assumindo `data.id` (remoteJid) + `data.presences[jid].lastKnownPresence`. Payload de tipo inesperado só é logado (`console.warn`) e ignorado — não derruba o webhook. **Ainda não testado contra tráfego real.**
- **`protocolMessage`** (edição/revogação): `REVOKE` assume `protocolMessage.key.id` apontando pra mensagem original. Edição assume `protocolMessage.type === 'MESSAGE_EDIT'` com `editedMessage.conversation`/`editedMessage.extendedTextMessage.text`. Formato desconhecido só loga e ignora (`continue`), nunca cria mensagem solta. **Ainda não testado contra tráfego real.**
- **`markMessageAsRead`**: implementado com `remoteJid` completo (não o número normalizado) no array `readMessages`, seguindo a mesma referência (`conv.wa_jid`) usada em outros pontos do código. **Ainda não testado.**
- **`sendPresence`**: implementado com body "flat" (`{number, presence, delay?}`). **Ainda não testado.**
...
- [ ] Presença enviada: digitar no CRM, conferir "digitando..." no WhatsApp do contato de teste; gravar áudio, conferir "gravando áudio...".
- [ ] Presença recebida: digitar no WhatsApp de teste, conferir "digitando..." no cabeçalho do CRM.
- [ ] Edição/revogação: editar e apagar uma mensagem do lado do WhatsApp de teste, conferir reflexo no CRM.
